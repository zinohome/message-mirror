...
func (mm *MirrorMaker) Start() error {
	mm.logger.Println("启动MirrorMaker...")
	...
	mm.logger.Printf("MirrorMaker已启动，使用 %d 个worker", mm.config.Mirror.WorkerCount)
	return nil
}
...
func (mm *MirrorMaker) worker(id int) {
	...
	mm.logger.Printf("Worker %d 启动", id)

	for {
		select {
		case <-mm.ctx.Done():
			mm.logger.Printf("Worker %d 停止", id)
			return

		case msg, ok := <-mm.source.Messages():
			if !ok {
				mm.logger.Printf("Worker %d: 消息通道已关闭", id)
				return
			}
			...
			if err := mm.applyConsumerRateLimit(msg); err != nil {
				...
				mm.logger.Printf("Worker %d: 应用消费速率限制失败: %v", id, err)
				continue
			}
			...
			if mm.batchProcessor != nil {
				...
				if err := mm.batchProcessor.Add(optMsg); err != nil {
					mm.logger.Printf("Worker %d: 添加消息到批处理器失败: %v", id, err)
					mm.stats.mu.Lock()
					mm.stats.Errors++
					mm.stats.mu.Unlock()
				}
				...
				// 否则直接处理单条消息
				if err := mm.processMessage(msg); err != nil {
					mm.logger.Printf("Worker %d: 处理消息失败: %v", id, err)
					mm.stats.mu.Lock()
					mm.stats.Errors++
					mm.stats.mu.Unlock()
...
func (mm *MirrorMaker) processMessage(msg *plugins.Message) error {
	...
	if mm.deduplicator != nil {
		...
		if err != nil {
			mm.logger.Printf("去重检查失败: %v", err)
			// 去重检查失败不影响消息处理，继续处理
		} else if isDuplicate {
			// 消息重复，跳过处理
			mm.logger.Printf("消息重复，已跳过: key=%s", string(msg.Key))
			// 仍然更新消费统计（因为确实消费了消息）
			mm.stats.mu.Lock()
			...
			if err := mm.source.Ack(msg); err != nil {
				mm.logger.Printf("确认重复消息失败: %v", err)
			}
			return nil
		}
	}
...
func (mm *MirrorMaker) Stop() error {
	mm.logger.Println("正在停止MirrorMaker...")
	...
	if mm.batchProcessor != nil {
		...
		mm.logger.Println("批处理器已停止")
	}

	// 停止数据源插件
	if err := mm.source.Stop(); err != nil {
		mm.logger.Printf("停止数据源插件失败: %v", err)
	}

	// 停止生产者
	if err := mm.producer.Stop(); err != nil {
		mm.logger.Printf("停止生产者失败: %v", err)
	}
	...
	if mm.logger != nil {
		if err := mm.logger.Stop(); err != nil {
			mm.logger.Printf("停止日志管理器失败: %v", err)
		}
	}
	...
	mm.logger.Println("MirrorMaker已停止")
	return nil
}
...
func (mm *MirrorMaker) OnConfigReload(oldConfig, newConfig *Config) error {
		...
		if mm.batchProcessor == nil {
			...
			processor := func(batch []*optimization.Message) error {
				for _, optMsg := range batch {
					...
					if err := mm.processMessage(msg); err != nil {
						mm.logger.Printf("批处理中处理消息失败: %v", err)
						// 继续处理其他消息
					}
				}
				return nil
	...
	if err := mm.producer.UpdateConfig(newConfig); err != nil {
		mm.logger.Printf("更新生产者配置失败: %v", err)
		// 不返回错误，因为其他配置已更新
	}

	mm.logger.Println("配置热重载完成")
	return nil
}
