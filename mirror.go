...
type MirrorMaker struct {
	...
	batchProcessor  *BatchProcessor  // 批处理器（如果启用）
	ctx             context.Context
...
func NewMirrorMaker(config *Config) (*MirrorMaker, error) {
	...
	// 创建数据源插件
	source, err := CreatePlugin(config.Source.Type)
	if err != nil {
		cancel()
	...
	// 创建日志管理器
	logger, err := NewLogger(&config.Log, ctx)
	if err != nil {
		cancel()
	...
	// 创建速率限制器
	var consumerLimiter, producerLimiter, bytesLimiter *RateLimiter
	...
	if config.Mirror.BytesRateLimit > 0 {
		...
		bytesLimiter = NewBytesRateLimiter(config.Mirror.BytesRateLimit, burstSize)
	} else {
		// 如果没有字节限制，使用消息限制
		if config.Mirror.ConsumerRateLimit > 0 {
			...
			consumerLimiter = NewRateLimiter(config.Mirror.ConsumerRateLimit, burstSize, true)
		}
		...
		if config.Mirror.ProducerRateLimit > 0 {
			...
			producerLimiter = NewRateLimiter(config.Mirror.ProducerRateLimit, burstSize, true)
		}
	}
	...
	deduplicator := NewDeduplicator(dedupConfig, ctx)
...
func (mm *MirrorMaker) worker(id int) {
	...
	for {
		...
		case msg, ok := <-mm.source.Messages():
			...
			// 如果启用批处理，将消息添加到批处理器
			if mm.batchProcessor != nil {
				if err := mm.batchProcessor.Add(msg); err != nil {
...
func (mm *MirrorMaker) applyConsumerRateLimit(msg *Message) error {
	// 优先使用字节速率限制器
...
func (mm *MirrorMaker) processMessage(msg *Message) error {
	startTime := time.Now()

	// 检查消息是否重复（去重）
	if mm.deduplicator != nil {
		isDuplicate, err := mm.deduplicator.IsDuplicate(msg)
		if err != nil {
...
func (mm *MirrorMaker) applyProducerRateLimit(msg *Message) error {
	// 优先使用字节速率限制器
	if mm.bytesLimiter != nil {
		return mm.bytesLimiter.WaitBytes(mm.ctx, len(msg.Value)+len(msg.Key))
	}
...
func (mm *MirrorMaker) getTargetTopic(msg *Message) string {
	// 如果配置了目标topic，直接使用
